...
const Review = () => {
  ...
  if (orderPlaced) {
    thankYouMessage = (
      <h4 className="order_placed_msg">
        Your order is placed successfully, Thanks.
      </h4>
    );
  }
  return (
      ...
      <div className="row">
        <div className="col-md-9">
          <div className="product_container">
            ...
            {thankYouMessage}
            {!cart.length && (
              <h4 className="cart-status text-danger">
                Cart is empty! <a href="/shop">Add products in your cart.</a>{" "}
              </h4>
            )}
          </div>
        </div>
            ...
            <Cart cart={cart}>
              <Link to="/shipping">
                {cart.length && auth.user ? (
                  <button className="cart_button btn btn-info">
                    Proceed Checkout
                  </button>
                ) : !auth.user ? (
                  <button className="cart_button btn btn-info">
